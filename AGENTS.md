...
- `js/main.js` coordinates the game loop and session state. You play one roommate (`game.player`); `career.js` handles their job, cash, rent and skills. Simulation lives in `sim.js`, `world.js`, `autonomy.js`, `interactions.js`, `traps.js`, `visitors.js`, `neighbours.js` (the families next door), `emergency.js` (fire brigade and police searches), `crime.js` (how witnesses react to your crimes, arrests, and talking witnesses round), `getaway.js` (the endgame when the last target dies with loose ends), `plans.js` (roommates' grudges, hunts and caution), `gang.js` (the biker gang), and `outsiders.js` (how non-roommates get hurt).
- `js/data.js` holds shared definitions; `js/contracts.js` defines campaign objectives, evaluation, and saved progress.
- `js/view.js`, `models.js`, `characters.js` (the animated figures), `lot.js`, `interior.js` (wallpaper, paintings, ceilings and lamps), `decor.js`, `garden.js`, and `effects.js` handle rendering and procedural assets; `js/ui.js` manages the interface.
- `sounds/` holds the recorded effects, ambience loops and title/pause music loaded by `js/audio.js` (which falls back to synthesised sounds). Raw clips go in the gitignored `sounds/originals/`; `python tools/build_sounds.py` (needs ffmpeg) trims, level-matches and renames them.
- `vendor/three.module.min.js` supplies Three.js through the HTML import map, and `vendor/addons/` holds the matching r170 add-ons (GLTFLoader, SkeletonUtils) mapped as `three/addons/`. Avoid hand-editing these dependencies.
- `models/` holds the animated figures loaded by `characters.js` (Kenney Mini Characters, CC0; see `models/LICENSE-kenney.txt`). The full asset pack goes in the gitignored `models/originals/`.
